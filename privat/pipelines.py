...
class PrivatPipeline:

    # Database setup
    conn = sqlite3.connect('privat.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `privat`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM privat WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO privat (date, title, link, content)"
